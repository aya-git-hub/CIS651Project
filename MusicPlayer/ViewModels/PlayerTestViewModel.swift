...
class PlayerTestViewModel: ObservableObject {
    ...
    /// 下载播放视图模型
    /// 用于：管理下载的音乐
    private let downloadPlayViewModel = DownloadPlayViewModel.getDownloadPlay()
    ...
    /// 加载资源音乐
    /// 用于：加载应用资源中的音乐文件
    private func loadResourcesMusic() {
        if let resourcePath = Bundle.main.resourcePath {
            do {
                let files = try FileManager.default.contentsOfDirectory(atPath: resourcePath)
                let musicFiles = files.filter { file in
                ...
                for musicFile in musicFiles {
                    if let url = Bundle.main.url(forResource: musicFile, withExtension: nil) {
                        ...
                        musicCache.setURL(url, for: musicFile)
                    }
                }
            } catch {
                handleError(.fileNotFound)
    ...
    private func loadDownloadedMusic() {
        ...
        for musicName in downloadPlayViewModel.downloadedItems {
    ...
    private func syncPlaybackState() {
        downloadPlayViewModel.currentPlayingMusic = currentMusicIndex >= 0 ? 
